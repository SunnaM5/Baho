...
export default function ProductPage({ params }: ProductPageProps) {
    ...
    ApiService.getProductBySlug(productId)
      .then((data) => {
        if (data && (data.id || data.slug)) {
          ...
          throw new Error('Product not returned directly')
        }
      })
      .catch(() => {
        ApiService.getProducts()
          .then((res) => {
            ...
            const found = list.find((p: any) => String(p.id) === String(productId) || p.slug === productId)
  ...
  if (loading) {
    return (
      <>
        <Header />
        <main className="min-h-screen bg-slate-50 flex items-center justify-center font-medium text-slate-500">
          Загрузка товара...
        </main>
        <Footer />
      </>
    ...
    return (
      <>
        ...
        <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
          <div className="text-center bg-white p-12 rounded-3xl border border-slate-200 shadow-sm max-w-md">
            <h1 className="text-2xl font-black text-slate-900 mb-2">Товар не найден</h1>
            <p className="text-slate-500 text-sm mb-6">Возможно, запрашиваемый товар был перемещен или удален.</p>
            <Link href="/catalog" className="inline-block px-6 py-3 bg-emerald-700 text-white font-bold text-sm rounded-xl">
              Вернуться в каталог
            </Link>
          </div>
        </main>
  ...
  // 1. Matrix Variant Matching (Terabayt logic: exact matching from backend SKU variants)
  const variants: any[] = product.variants || []
  
  const matchedVariant = variants.find((v) => {
    const memMatch = !selectedStorageObj || String(v.memory_id) === String(selectedStorageObj.id)
    const simMatch = !selectedSimObj || String(v.sim_id) === String(selectedSimObj.id)
    const colorMatch = !selectedColorObj || String(v.color_id) === String(selectedColorObj.id)
    return memMatch && simMatch && colorMatch && v.is_active
  })

  // Fallback price logic from backend Source-Of-Truth
  ...
  // Gallery images matching selected color or all product images
  ...
  // Helper functions for disabled state checks (Matrix Availability)
  ...
  // Calculate installment monthly payment based on selected months and calculated price (prioritizing matched SKU variant)
  const customInstallmentPriceMap: Record<number, number | undefined> = {
    3: matchedVariant?.installment_3m_price ? Number(matchedVariant.installment_3m_price) : (product?.installment_3m_price ? Number(product.installment_3m_price) : undefined),
    6: matchedVariant?.installment_6m_price ? Number(matchedVariant.installment_6m_price) : (product?.installment_6m_price ? Number(product.installment_6m_price) : undefined),
    12: matchedVariant?.installment_12m_price ? Number(matchedVariant.installment_12m_price) : (product?.installment_12m_price ? Number(product.installment_12m_price) : undefined),
  }
  const customPrice = customInstallmentPriceMap[selectedMonths]

  const activePlan = installmentMonths.find((item) => item.months === selectedMonths) || installmentMonths[2]
  const totalInstallmentPrice = Math.round(calculatedPrice * activePlan.markup)
  const monthlyPaymentRaw = customPrice && customPrice > 0
    ? Math.round(customPrice)
    : Math.round(totalInstallmentPrice / selectedMonths)
    ...
    <>
      ...
      <main className="min-h-screen bg-slate-50 py-8">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            >
              <div className="bg-white rounded-3xl border border-slate-200/80 p-6 md:p-8 shadow-sm space-y-6">
                ...
                {/* Storage / Memory Selector (Xotira) */}
                ...
                {/* Installment Months Interactive Calculator */}
                ...
                <div className="space-y-3 pt-2">
                  ...
                  {/* Buy in 1-Click / Apply Installment Button */}
                  <button
          ...
          {/* Full Specifications & Product Description */}
          ...
          >
            ...
            {orderSuccess ? (
              <div className="py-8 text-center space-y-3">
                ...
                <h3 className="text-xl font-black text-slate-900">Заявка принята!</h3>
              ...
              <form onSubmit={handleSubmitQuickOrder} className="space-y-4">
                ...
                <p className="text-xs text-slate-500">
                  Оставьте ваш номер телефона для быстрого оформления рассрочки на {product.name} ({selectedMonths} мес).
                </p>
